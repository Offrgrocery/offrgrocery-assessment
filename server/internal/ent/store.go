...
type Store struct {
	...
	// Grocer holds the value of the "grocer" field.
	Grocer store.Grocer `json:"grocer,omitempty"`
...
func (*Store) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case store.FieldStoreID, store.FieldGrocer:
...
func (_m *Store) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case store.FieldGrocer:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field grocer", values[i])
			} else if value.Valid {
				_m.Grocer = store.Grocer(value.String)
			}
		default:
...
func (_m *Store) QueryItems() *ItemQuery {
	return NewStoreClient(_m.config).QueryItems(_m)
}
...
func (_m *Store) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("grocer=")
	builder.WriteString(fmt.Sprintf("%v", _m.Grocer))
	builder.WriteByte(')')
